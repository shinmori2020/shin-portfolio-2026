...
**追加の改善（2026-07-29）:** `/works` と作品詳細で LCP になる画像を先読みに変更した。
どちらも待ち時間の大半が Load Delay（読み込みを始めるまで）で ダウンロード自体は
57〜184ms で終わっていた。既定の遅延読み込みだとブラウザが画像の必要性に気づくのが遅れる。

| ページ | Perf | LCP |
|---|---|---|
| `/works` | 86 → **93〜96** | 4.1s → **2.7〜3.0s** |
| `/works/[slug]` | 82 → **78〜94**（平均 88 前後） | 4.4s → 2.9〜4.4s |

**残る改善余地（任意）:** 作品詳細の Render Delay が 1.7〜3.7秒と振れる。
ヒーロー画像を包む reveal のフェード（0.75秒）がそのまま LCP に乗るため、
この1箇所だけ演出を外せば縮む。ただし見出しは動いて画像だけ即出るため
見た目の統一感は落ちる。表示をブロックする CSS（34KB）を削る手もある。
