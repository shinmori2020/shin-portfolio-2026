...
### 5. 公開前チェック — SEO と a11y は完了 / 残りは速度

`.claude/skills/` の4スキルに沿って通しで監査する。

| スキル | 状態 |
|---|---|
| `seo` | **完了**（2026-07-29）。sitemap / robots / canonical / 構造化データを追加し `/works` の見出し階層の飛びを修正 |
| `accessibility` | **完了**（2026-07-29）。axe-core を全7ページ×ライト/ダークで実行。重大2件を修正し 残るコントラストは上記のとおり現状維持と判断 |
| `core-web-vitals` | **未実施**。LCP / INP / CLS の実測 |
| `web-quality-audit` | **未実施**。総合 |
