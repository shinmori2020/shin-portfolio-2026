...
1. **プレースホルダーが半透明（透ける）**
   斜線プレースホルダーは `repeating-linear-gradient(..., var(--surface-2), transparent, ...)` で**縞の隙間が透明**だった。下地が無いため、要素の背後が透けて見える。
2. **暗いスクリムが旧スナップショットに焼き込まれ、透けて出る**
   Home カードの共有要素の箱の**内側**に、ホバーで出る「View →」の暗い半透明スクリム（`rgba(15,18,14,0.45)`）がある。クリック時はカーソルがホバー中なのでスクリムが最大表示のまま morph の**旧スナップショットへ焼き込まれる**。morph 中は手前の新スナップショット（詳細ヒーロー）に透明の隙間があるため、**その隙間から奥の暗い旧スナップショット（スクリム）が透けて縞になる**。

Works一覧カードは `BrowserFrame` 製で共有要素の箱に**スクリムが無い**ため、透ける奥も明るい斜線だけで問題にならなかった。これが「一覧からは綺麗・Homeからは縞々」の差の正体。＝本質は仮画像（半透明プレースホルダー）特有の症状で、不透明な実スクリーンショット投入後は自然に起きない。

> 経緯メモ：当初は「縦横比ズレによる端の隙間」と誤診し `object-fit:cover` で解決としたが、真因は上記の透過＋スクリム焼き込みだったため縞は残っていた。

### 対策（A案 = 1+2）

1. **プレースホルダーに不透明な下地（surface）を敷き、透過を無くす**（根治）
   斜線グラデーションの最終レイヤーに `var(--surface)` を追加し、隙間を不透明にする。新スナップショットが不透明になり、奥のスクリムが透けなくなる。全プレースホルダー箇所（`BrowserFrame` = 一覧/詳細ヒーロー、Home カード、`LatestWorkCard` の `.hlw-shot-inner`）に適用。
2. **`view-transition-name` をスクリムを含む箱からスクリーンショット画像そのものへ移す**
   Home カードは、名前をスクリーンショット画像の div に付け、暗いスクリム（`View →` オーバーレイ）を共有要素の外に置く。移動元スナップショットにスクリムが焼き込まれなくなる。（`BrowserFrame` は元々スクリムを含まないので移動不要）

補足：前回入れた `object-fit:cover`（下記）は**縦横比ズレ（一覧16/10 ↔ 詳細16/8.5）で端に隙間を作らない**対策として有効なので残置している。
...
::view-transition-new(.work-shot) {
  width: 100%;
  height: 100%;
  object-fit: cover; /* 縦横比が違っても端に隙間を作らず覆い切る */
  animation: none;   /* 即入替（二重写りのゴースト防止） */
  mix-blend-mode: normal;
}
```

1（不透明化）で透過そのものを断ち、2（スクリム除外）で焼き込みも断つ二重の対策。実スクリーンショット投入後も同じ構造でそのまま効く。
